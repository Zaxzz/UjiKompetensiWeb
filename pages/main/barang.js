...
import { ArrowUpward, Delete, Person, AdminPanelSettings, ArrowDownward, Add, Close, Edit} from "@mui/icons-material";
...
function Patch({nama, harga_awal, token ,deskripsi ,id, imagefrom}) {
  const [open, setOpen] = useState(false);
  const [data,setData] = useState({
    token: token,
    method:"patch",
  ...
  <div className="bg-white w-[auto] h-[auto] rounded-lg border-black p-[16px] flex flex-col gap-6">
    ...
    <div className="flex flex-row gap-3 mb-2">
    <div className="gap-[61px] flex"><label>Image </label> <label>:</label></div> <div className="flex flex-col gap-3"> <input type="file" name="image" onChange={handleChange} ></input>{ createObjectURL === null ? <Image src={`/upload/${imagefrom}`} width={200} height={200} />: <Image src={createObjectURL} width={200} height={200} />}</div>
    </div>
    <div className="flex flex-row gap-3">
    <label>Harga Barang : </label> <div className="flex flex-col gap-3"> <input type="number" name="harga_awal" value={data?.harga_awal} onChange={handleChange} className="border-black bg-gray-200 rounded-md p-[8px]" placeholder={rupiah(data?.harga_awal)} ></input> <p className="text-xs text-gray-500">{rupiah(data?.harga_awal)}</p> </div>
    </div>
          ...
          <tr  key={data.id_pengaduan} class=" border-b  dark:border-gray-300 border-gray-300 dark:hover:bg-zinc-200">
             ...
             {data?.nama_barang }
                ...
                <td class=" pl-6 py-4">
                    ...
                    <Patch id={data?.id_barang} token={user?.token} nama={data?.nama_barang} harga_awal={data?.harga_awal} deskripsi={data?.deskripsi}  imagefrom={data?.image}/>
